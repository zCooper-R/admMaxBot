...
- `CI` запускается на `push` и на `pull_request` в `main`;
- pipeline выполняет `python manage.py check` и полный `pytest -q`;
- `Lint` запускается на `pull_request` в `main` и проверяет только изменённые Python-файлы через `ruff` и `black --check`;
...
6. Рабочие ветки для изменений создаём от `main` и называем по смыслу задачи без обязательных префиксов.
